...
export default class Rubix {
  constructor(id, opts) {
    ...
    this.$.fn.serializeObject=function(){"use strict";var a={},b=function(b,c){var d=a[c.name];"undefined"!=typeof d&&d!==null?$.isArray(d)?d.push(c.value):a[c.name]=[d,c.value]:a[c.name]=c.value};return $.each(this.serializeArray(),b),a};
  ...
  draw() {
    ...
    this._draw(this.$(window).width(), this.$(window).height());
  }
  ...
  /** @private */
  setupOpts() {
    this.opts.theme_style = this.opts.theme_style || this.$('body').attr('data-theme') || 'light';
    ...
    if (this.master_detail) {
      ...
      this.brush.on('brush', () => {
        ...
        if (type === 'mousemove' || type === 'touchmove') {
          this.$(window).trigger('rubix.sidebar.off');
  ...
  setupRedraw() {
    ...
    this.$(window).on('orientationchange.' +this.masterId, () => {
      self.draw();
    });

    this.$(window).on('rubix.redraw.' +this.masterId, () => {
      self.draw();
    });

    switch (this.resize) {
      case 'always':
      this.$(window).on('resize.rubix.' +this.masterId, () => {
        self.draw();
      });
      break;
      case 'debounced':
      this.$(window).on('debouncedresize.rubix.' +this.masterId, () => {
        self.draw();
      });
      break;
      case 'throttled':
      this.$(window).on('throttledresize.rubix.' +this.masterId, () => {
        self.draw();
      });
      break;
      default:
  ...
  */
  ...
  move_tooltip_x(dx, ys, points) {
    ...
    this.tooltip.css('display', this.useTable);
    this.tooltip.css({
      'left': left,
      'top' : top
    });
  ...
  move_tooltip_y(dy, yx, points) {
    try {
      ...
      let _elem = this.$(this.canvas.node());
  ...
  resetFocus(forced) {
    ...
    //
    // let out = () => {
    //   this.$(window).trigger('rubix.sidebar.on');
    //   d3.event.preventDefault();
    //   this.$(this).focus();
    //   self.tooltip.hide();
    //   for(let name in self.charts) {
  ...
  resetLabelHandlers() {
    let self = this;
    this.$('.' +this.masterId+'-legend-labels').css({
      'opacity': 0.9
    });

    this.$('.' +this.masterId+'-legend-labels').off().on({
      'hover': function(e) {
        let hasClass = this.$(this).hasClass('toggle');
      ...
      'click': function(e) {
        let hasClass = this.$(this).hasClass('toggle');
        let name = this.$(this).attr('data-name');
        let type = this.$(this).attr('data-type') || '';

        if (!hasClass) {
          this.$(this).css({
            'opacity': 0.2
          });
          this.$(this).addClass('toggle');
  ...
  */
  ...
  Cleanup() {
    ...
    for(let i = 0; i < RubixListeners.length; i++) {
      ...
      this.$(window).off('orientationchange.' + masterId);
      this.$(window).off('rubix.redraw.' + masterId);
      this.$(window).off('resize.rubix.' + masterId);
      this.$(window).off('debouncedresize.rubix.' + masterId);
      this.$(window).off('throttledresize.rubix.' + masterId);
    }
    RubixListeners = [];
  };
